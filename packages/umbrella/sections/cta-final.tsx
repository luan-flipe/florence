...
import { useCursoInteresse } from "../curso-interesse-context";
import type { UmbrellaConfig, UtmParams } from "../types";

interface CtaFinalProps {
  ctaFinal: UmbrellaConfig["ctaFinal"];
  formulario: UmbrellaConfig["formulario"];
  cursoOptions: string[];
  utm?: UtmParams;
}

export function CtaFinal({ ctaFinal, formulario, cursoOptions, utm }: CtaFinalProps) {
    ...
    <section ref={ref} className="relative py-24 lg:py-32 overflow-hidden">
      ...
      <div className="relative z-10 container mx-auto px-4 lg:px-8">
        <div className="max-w-xl mx-auto text-center mb-12">
          <h2 className="headline-lg text-3xl lg:text-5xl text-white mb-4 reveal text-balance">{ctaFinal.headline}</h2>
          <p className="text-white/70 text-lg reveal reveal-delay-1">{ctaFinal.microcopy}</p>
        </div>
        <div className="max-w-md mx-auto reveal reveal-delay-2">
          <Formulario
            ...
            extraFields={[{ name: "curso_interesse", label: formulario.cursoLabel, type: "select", options: cursoOptions, required: true, placeholder: formulario.cursoPlaceholder }]}
