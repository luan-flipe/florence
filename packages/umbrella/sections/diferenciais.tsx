...
interface DiferenciaisProps {
  diferenciais: UmbrellaConfig["diferenciais"];
}

export function Diferenciais({ diferenciais }: DiferenciaisProps) {
  const ref = useScrollReveal<HTMLElement>();
  const d = diferenciais;
      ...
      <div className="container mx-auto px-4 lg:px-8">
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-start">
          ...
          <div className="reveal">
            <div className="eyebrow eyebrow-dark mb-5 w-fit"><span className="w-1.5 h-1.5 rounded-full bg-[#F5C842]" />{d.eyebrow}</div>
            <div className="flex items-end gap-4 lg:gap-5 mb-5">
              <FlorenceIcon className="h-[4.25rem] lg:h-[6.75rem] w-auto flex-shrink-0" />
              <h2 className="headline-lg text-3xl lg:text-5xl text-white whitespace-pre-line">{d.lead.titulo}</h2>
            </div>
